...
moment=1.                   # source dipole moment
receiver_orientation = "z"  # Rx orientation
data_type = "ppm"           # data unit


# true model:
model_filename = "./outputs/em1dfm_truemodel.txt"
...
uncertainties =rel_err *np.abs(dobs)*np.ones(np.shape(dobs)) + noise_level

# Define the data object
#data_object = data.Data(survey, dobs=dobs, noise_floor=noise_level, standard_deviation=uncertainties)
...
plot_layer(true_model, true_layers, ax=ax2, showlayers=False, plotSig=False, color="k", label="True Model")
if plotL2:
...
ax1.loglog(frequencies, np.abs(dobs[0::2]), "k-o", label="Observed (real)")
ax1.loglog(frequencies, np.abs(dobs[1::2]), "k:o", label="Observed (imag)")
if plotL2:
    ax1.loglog(frequencies, np.abs(dpred_l2[0::2]), "b-o", label="L2-Model (real)")
    ax1.loglog(frequencies, np.abs(dpred_l2[1::2]), "b:o", label="L2-Model (imag)")
if plotSparse:
    ax1.loglog(frequencies, np.abs(dpred_final[0::2]), "r-o", label="Sparse (real)")
    ax1.loglog(frequencies, np.abs(dpred_final[1::2]), "r:o", label="Sparse (imag)")
ax1.plot(frequencies, noise_level*np.ones(frequencies.shape), 'k--')
